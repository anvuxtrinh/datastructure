...
void test_cstr_appendn_newsize_exceeds_cap() {
    ...
    ASSERT_TRUE(str.cap >= 6, "Expected condition to be true");

    ret = cstr_appendn(&str, " World", 6);
    ASSERT_EQ(CSTR_SUCCESS, ret, "Expected condition to hold");
    ASSERT_STR_EQ("Hello World", str.data, "Expected strings to match");
    ASSERT_EQ(11, str.len, "Expected condition to hold");
    ASSERT_TRUE(str.cap >= 12, "Expected condition to be true");
}
...
void test_cstr_appendn_within_cap() {
    ...
    ASSERT_EQ(CSTR_SUCCESS, ret, "Expected condition to hold");
    ASSERT_STR_EQ("Hello!", str.data, "Expected strings to match");
    ASSERT_EQ(6, str.len, "Expected condition to hold");
    ASSERT_TRUE(str.cap >= 7, "Expected condition to be true");
}
